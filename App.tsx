...
const getSourceIcon = (source: LocalDatabase['source']) => {
  switch (source) {
    case 'public': return <Cloud className="w-4 h-4 text-emerald-600" />;
    ...
    return (
      <div className="min-h-screen bg-white flex flex-col items-center justify-center p-6 text-center">
         <div className="relative mb-6">
            ...
            <RefreshCw className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-6 h-6 text-emerald-600" />
          </div>
          <p className="text-emerald-600 font-black tracking-widest animate-pulse text-[10px] uppercase">Sincronizando MaestroDB...</p>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-white text-slate-900 selection:bg-emerald-50 selection:text-emerald-900">
      <nav className="bg-white/90 backdrop-blur-xl border-b border-slate-100 px-6 py-4 flex items-center justify-between sticky top-0 z-50 shadow-sm">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-xl font-black tracking-tighter bg-gradient-to-r from-slate-900 to-emerald-600 bg-clip-text text-transparent">MaestroDB</h1>
        </div>
        <div className="flex gap-1 bg-slate-100 p-1 rounded-2xl border border-slate-200">
          {[
            ...
            <button key={tab.id} onClick={() => setActiveTab(tab.id)} className={`flex items-center gap-2 px-4 py-2 rounded-xl transition-all ${activeTab === tab.id ? 'bg-white text-emerald-600 shadow-sm' : 'text-slate-500 hover:text-slate-900 hover:bg-slate-200'}`}>
      ...
      <main className="max-w-6xl mx-auto p-6 md:p-8">
        {status.type && (
          <div className={`mb-6 p-4 rounded-xl flex items-center gap-3 border animate-in slide-in-from-top-4 ${status.type === 'success' ? 'bg-emerald-50 border-emerald-100 text-emerald-700' : 'bg-red-50 border-red-100 text-red-700'}`}>
          ...
          <div className="space-y-8 animate-in fade-in duration-500">
            <header className="flex flex-col md:flex-row md:items-center justify-between gap-4">
              ...
              <div className="flex items-center gap-3 bg-slate-50 px-4 py-2.5 rounded-2xl border border-slate-100">
                ...
                <span className="text-[10px] font-black text-slate-500 uppercase tracking-widest">{getSourceLabel(db.source)}</span>
              </div>
            </header>

            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <div className="bg-white border border-slate-100 p-8 rounded-[2.5rem] shadow-xl relative overflow-hidden group hover:border-emerald-200 transition-all">
              ...
              <div className="bg-white border border-slate-100 p-8 rounded-[2.5rem] shadow-xl relative overflow-hidden group hover:border-red-200 transition-all">
                ...
                <p className="text-3xl font-black text-red-600 tabular-nums tracking-tight">{formatCOP(metrics.negativeAdj)}</p>
                <p className="text-[10px] text-slate-400 mt-2 font-bold uppercase">Costo total de stock negativo</p>
              </div>

              <div className="bg-white border border-slate-100 p-8 rounded-[2.5rem] shadow-xl relative overflow-hidden group hover:border-sky-200 transition-all">
                <div className="absolute top-0 right-0 p-4 opacity-[0.05] group-hover:opacity-10 transition-opacity">
                   <TrendingUp className="w-24 h-24 text-sky-600" />
                </div>
                <p className="text-slate-400 text-[10px] font-black uppercase tracking-widest mb-1">Ajuste General</p>
                <p className="text-3xl font-black text-slate-900 tabular-nums tracking-tight">{formatCOP(metrics.totalAdj)}</p>
              ...
              <div className="px-10 py-10 border-b border-slate-50 bg-slate-50/30">
                ...
                <div className="flex flex-wrap items-center gap-3">
                  ...
                  <button
                    ...
                    className="flex items-center gap-2 px-5 py-3 rounded-xl border border-slate-100 bg-slate-50 text-slate-400 hover:text-red-600 hover:border-red-100 transition-all text-[11px] font-black uppercase tracking-tight ml-auto"
                  >
                    <X className="w-4 h-4" />
                    Limpiar Filtros
                  </button>
                ...
                {showFecha && (
                  <div className="mt-8 p-8 bg-slate-50 rounded-[2rem] flex flex-wrap gap-10 animate-in slide-in-from-top-6 duration-400 border border-slate-100">
                    <div className="space-y-3">
                      <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest block ml-1">Rango Desde</label>
                      <input type="date" value={desde} onChange={(e) => setDesde(e.target.value)} className="bg-white border border-slate-200 rounded-xl px-5 py-3 text-xs text-slate-900 focus:outline-none focus:ring-4 focus:ring-emerald-50 transition-all" />
                    </div>
                    <div className="space-y-3">
                      <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest block ml-1">Rango Hasta</label>
                      <input type="date" value={hasta} onChange={(e) => setHasta(e.target.value)} className="bg-white border border-slate-200 rounded-xl px-5 py-3 text-xs text-slate-900 focus:outline-none focus:ring-4 focus:ring-emerald-50 transition-all" />
                    </div>
                  </div>
                )}
              </div>
                  ...
                  <thead>
                    <tr className="bg-slate-50/50">
                      {visibleHeaders.map((h) => (
                        <th key={h} className="px-10 py-6 text-[10px] font-black text-slate-400 uppercase tracking-widest border-b border-slate-100 whitespace-nowrap">
                          {h}
                        </th>
                      ))}
                    </tr>
                  ...
                  <tbody>
                    {filteredRows.slice(0, 50).map((row, i) => (
                      <tr key={i} className="hover:bg-slate-50 transition-colors group">
                        {visibleHeaders.map((h) => (
                          <td key={h} className="px-10 py-5 text-[13px] text-slate-600 border-b border-slate-50 whitespace-nowrap group-hover:text-slate-900">
                            {String(row[h] ?? "")}
                          </td>
                        ))}
                      </tr>
            ...
            {!analysis ? (
              <div className="bg-white border border-slate-100 rounded-[2.8rem] p-24 text-center shadow-2xl relative overflow-hidden">
                <div className="bg-indigo-50 w-28 h-28 rounded-full flex items-center justify-center mx-auto mb-10">
                  <Zap className="w-14 h-14 text-indigo-600" />
                </div>
                <h2 className="text-4xl font-black text-slate-900 mb-6 tracking-tight">IA Predictiva Maestro</h2>
                <p className="text-slate-500 mb-12 max-w-sm mx-auto leading-relaxed font-medium">Gemini Pro analizará {db.rows.length.toLocaleString()} referencias para detectar fugas críticas y patrones de stock.</p>
                <button onClick={handleAnalyze} disabled={loading} className="bg-indigo-600 hover:bg-indigo-700 text-white font-black py-6 px-16 rounded-[2.5rem] flex items-center gap-4 mx-auto shadow-2xl shadow-indigo-200 active:scale-95 transition-all uppercase text-[11px] tracking-[0.2em]">
              ...
              <div className="space-y-8">
                <div className="flex items-center justify-between">
                  <h2 className="text-2xl font-black text-slate-900 flex items-center gap-4 uppercase tracking-widest text-[11px]"><Zap className="text-indigo-600 w-6 h-6" /> Informe de Inteligencia</h2>
                ...
                <div className="bg-white border border-indigo-100 rounded-[3rem] p-16 shadow-2xl relative overflow-hidden">
                  <div className="absolute top-0 right-0 p-12 opacity-[0.05] pointer-events-none">
                     <Zap className="w-64 h-64 text-indigo-600" />
                  </div>
                  <p className="text-2xl text-slate-700 leading-relaxed italic border-l-8 border-indigo-600 pl-14 mb-16 font-medium font-serif">"{analysis.summary}"</p>
                  <div className="grid lg:grid-cols-2 gap-16">
                    <div className="bg-slate-50 p-12 rounded-[2.5rem] border border-slate-100">
                      <h4 className="text-[10px] font-black text-indigo-600 uppercase tracking-widest mb-10 flex items-center gap-3"><Search className="w-4 h-4"/> Hallazgos del Sistema</h4>
                      <ul className="space-y-8">
                        {analysis.insights.map((ins, i) => (
                          <li key={i} className="text-[15px] text-slate-600 flex gap-6 leading-relaxed"><span className="text-indigo-600 font-black bg-indigo-50 w-9 h-9 rounded-xl flex items-center justify-center shrink-0 border border-indigo-100 shadow-sm">{i+1}</span> {ins}</li>
                        ))}
                      </ul>
                    </div>
                    <div className="bg-emerald-50 p-12 rounded-[2.5rem] border border-emerald-100">
                      <h4 className="text-[10px] font-black text-emerald-600 uppercase tracking-widest mb-10 flex items-center gap-3"><CheckCircle2 className="w-4 h-4"/> Plan de Acción</h4>
                      <ul className="space-y-8">
                        {analysis.suggestedActions.map((action, i) => (
                          <li key={i} className="text-[15px] text-slate-700 flex gap-6 leading-relaxed"><CheckCircle2 className="w-7 h-7 text-emerald-600 shrink-0 mt-0.5" /> {action}</li>
                        ))}
                      </ul>
                    </div>
                  </div>
        ...
        {activeTab === 'settings' && (
          <div className="max-w-2xl mx-auto space-y-8 animate-in slide-in-from-bottom-6 duration-500">
            <div className="bg-white border border-slate-100 rounded-[3rem] p-16 shadow-2xl relative overflow-hidden">
              ...
              <p className="text-slate-500 text-sm mb-14 font-medium leading-relaxed">Administración del entorno LiquorHub y sincronización de nodos de datos locales.</p>
              <div className="space-y-10">
                <div className="bg-slate-50 border border-slate-100 rounded-[2.5rem] p-12 flex items-center justify-between shadow-inner group">
                  <div className="flex items-center gap-8">
                    <div className="bg-white p-6 rounded-3xl border border-slate-100 group-hover:scale-110 transition-transform shadow-sm">
                      <FileSpreadsheet className="w-11 h-11 text-emerald-600" />
                    </div>
                    <div>
                ...
                <div className="p-12 bg-white border border-slate-100 rounded-[2.5rem] text-[10px] space-y-8 shadow-sm">
                   <div className="flex justify-between items-center border-b border-slate-50 pb-6">
                      <span className="text-slate-400 uppercase font-black tracking-widest">Origen de Transmisión</span>
                    ...
                    <div className="flex justify-between items-center border-b border-slate-50 pb-6">
                      <span className="text-slate-400 uppercase font-black tracking-widest">Ofuscación de Columnas</span>
                    ...
                    <div>
                      <span className="text-slate-400 uppercase font-black tracking-widest block mb-6">Políticas de Ocultación Activas:</span>
      ...
      {loading && (
        <div className="fixed inset-0 bg-white/95 backdrop-blur-2xl z-[100] flex flex-col items-center justify-center">
          <div className="relative mb-12">
            <div className="w-32 h-32 border-b-4 border-emerald-600 rounded-full animate-spin"></div>
            <RefreshCw className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-12 h-12 text-emerald-600 animate-pulse" />
          </div>
